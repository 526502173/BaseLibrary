...
class PagingActivity : LibraryBaseListActivity() {
    ...
    private fun initRecyclerView() {
        ...
        rv_article.addItemDecoration(VerticalItemDecoration(0.5.dp2px(this), Color.parseColor("#e0e0e0")) { _, position ->
            position != mViewModel.articleList.value?.size
        })
    ...
    private fun bindViewModel() {
        ...
        mViewModel.loadMoreStatus.observe(this, Observer {
            when (it.status) {
                UIStatus.LOAD_MORE_NORMAL -> mAdapterWrapper.normal()
                UIStatus.LOAD_MORE_NO_MORE -> mAdapterWrapper.noMore()
            }
        })
        mViewModel.networkStatus.observe(this, Observer {
            when (it.status) {
                UIStatus.SUCCESS -> showSuccess()
                UIStatus.FAILED -> showLoadFailed(it.failCode)
